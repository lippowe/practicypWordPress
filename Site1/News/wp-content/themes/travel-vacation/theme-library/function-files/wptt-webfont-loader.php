...
if ( ! class_exists( 'WPTT_WebFont_Loader' ) ) {
	...
	class WPTT_WebFont_Loader {
		 ...
		 * @var string
		 */
		protected $travel_vacation_remote_styles;
		...
		public function get_remote_url_contents() {
			...
			// Get the response.
			$travel_vacation_response = wp_remote_get( $this->remote_url, array( 'user-agent' => $user_agent ) );

			// Early exit if there was an error.
			if ( is_wp_error( $travel_vacation_response ) ) {
				return '';
			}

			// Get the CSS from our response.
			$travel_vacation_contents = wp_remote_retrieve_body( $travel_vacation_response );

			return $travel_vacation_contents;
		}
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $travel_vacation_font_family => $files ) {

				// The folder path for this font-family.
				$folder_path = $this->get_fonts_folder() . '/' . $travel_vacation_font_family;
			...
			if ( $change ) {
				...
				foreach ( $stored as $travel_vacation_url => $travel_vacation_path ) {
					if ( ! file_exists( $travel_vacation_path ) ) {
		...
		public function get_remote_files_from_css() {
			...
			$travel_vacation_font_faces = explode( '@font-face', $this->remote_styles );

			$travel_vacation_result = array();

			// Loop all our font-face declarations.
			foreach ( $travel_vacation_font_faces as $travel_vacation_font_face ) {

				// Make sure we only process styles inside this declaration.
				$style = explode( '}', $travel_vacation_font_face )[0];
				...
				preg_match_all( '/font-family.*?\;/', $style, $travel_vacation_matched_font_families );

				// Get an array of our font-files.
				preg_match_all( '/url\(.*?\)/i', $style, $matched_font_files );

				// Get the font-family name.
				$travel_vacation_font_family = 'unknown';
				if ( isset( $travel_vacation_matched_font_families[0] ) && isset( $travel_vacation_matched_font_families[0][0] ) ) {
					$travel_vacation_font_family = rtrim( ltrim( $travel_vacation_matched_font_families[0][0], 'font-family:' ), ';' );
					$travel_vacation_font_family = trim( str_replace( array( "'", ';' ), '', $travel_vacation_font_family ) );
					$travel_vacation_font_family = sanitize_key( strtolower( str_replace( ' ', '-', $travel_vacation_font_family ) ) );
				}

				// Make sure the font-family is set in our array.
				if ( ! isset( $travel_vacation_result[ $travel_vacation_font_family ] ) ) {
					$travel_vacation_result[ $travel_vacation_font_family ] = array();
				}
				...
				foreach ( $matched_font_files as $match ) {
					...
					// Add the file URL.
					$travel_vacation_font_family_url = rtrim( ltrim( $match[0], 'url(' ), ')' );

					// Make sure to convert relative URLs to absolute.
					$travel_vacation_font_family_url = $this->get_absolute_path( $travel_vacation_font_family_url );

					$travel_vacation_result[ $travel_vacation_font_family ][] = $travel_vacation_font_family_url;
				}

				// Make sure we have unique items.
				// We're using array_flip here instead of array_unique for improved performance.
				$travel_vacation_result[ $travel_vacation_font_family ] = array_flip( array_flip( $travel_vacation_result[ $travel_vacation_font_family ] ) );
			}
			return $travel_vacation_result;
		}
		...
		protected function get_absolute_path( $travel_vacation_url ) {
			...
			if ( 0 === stripos( $travel_vacation_url, '/' ) ) {
				...
				return $parsed_url['scheme'] . '://' . $parsed_url['hostname'] . $travel_vacation_url;
			}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	 ...
	 */
	function wptt_get_webfont_styles( $travel_vacation_url, $format = 'woff2' ) {
		$font = new WPTT_WebFont_Loader( $travel_vacation_url );
...
if ( ! function_exists( 'wptt_get_webfont_url' ) ) {
	 ...
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_url( $travel_vacation_url, $format = 'woff2' ) {
		$font = new WPTT_WebFont_Loader( $travel_vacation_url );
		$font->set_font_format( $format );
		return $font->get_url();
	}
